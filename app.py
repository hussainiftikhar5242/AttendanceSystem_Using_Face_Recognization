...
def remove_empty_rows():
    input_file = 'UserList/Unregistered.csv'
    output_file = 'UserList/Unregistered.csv'
...
def remove_empty_cells():
    # Remove empty cells from Registered.csv
    dfr = pd.read_csv('UserList/Registered.csv')
    dfr.dropna(inplace=True)
    dfr.to_csv('c', index=False)

    # Remove empty cells from Unregistered.csv
    dfu = pd.read_csv('UserList/Unregistered.csv')
    dfu.dropna(inplace=True)
    dfu.to_csv('UserList/Unregistered.csv', index=False)

    # Remove empty cells from all CSV files in the Attendance folder
    for file in os.listdir('Attendance'):
        if file.endswith('.csv'):
            csv_path = os.path.join('Attendance', file)
...
def registeruser():
    ...
    shutil.move('static/faces/' + str(dfu.iloc[idx]['Name'] )+ '$' + str(dfu.iloc[idx]['ID']) + str('$None'),
                'static/faces/' + str(dfu.iloc[idx]['Name']) + '$' + str(dfu.iloc[idx]['ID']) + '$' +str(sec))
    ...
    # Instead of appending a single row, append the entire DataFrame dfu to dfr
    dfr = pd.concat([dfr, dfu.iloc[[idx]]], ignore_index=True)
